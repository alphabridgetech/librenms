...
class MibsUploadController extends Controller
{
    public function index()
    {
        
        // adjust policy name if you use a different ability
        $this->authorize('viewAny', CustomMib::class);

        $mibs = CustomMib::with(['uploader:user_id,username'])
            ->orderByDesc('created_at')
            ->get();

        // device list for dropdown
        $devices = Device::orderBy('hostname')->get(['device_id', 'hostname', 'sysName','sysObjectID']);
    

        
        return view('mibs.index', compact('mibs', 'devices'));
    }
